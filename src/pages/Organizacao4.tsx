// Notion-like minimal editor with nested pages, blocks, multi‑select and drag‑to‑move
// This component implements a simplified Notion clone in React.
// Features:
//  - Create pages and nested subpages (pages can contain other pages) similar to Notion.
//  - Each page has a title and a list of text blocks (one line per block).
//  - Enter creates a new block below the current one; Backspace removes empty blocks.
//  - Shift+Up/Down extends selection; Ctrl/Cmd+Click toggles selection; Shift+Click selects a range.
//  - Drag across block rows to select multiple blocks.
//  - Drag selected blocks onto a page in the sidebar to move them to that page.
//  - Minimalistic layout with Tailwind classes.
...
// Types for clarity (you can strip these if using JS)
type Block = {
  id: string;
  content: string;
};
...
export default function Organizacao4() {
  // SEO Configuration
  useEffect(() => {
    setPageSEO(
      "Organização 4 - Editor Notion-like",
      "Editor de blocos com páginas aninhadas, seleção múltipla e arrastar para mover, estilo Notion."
    );
  }, []);

  // Load from localStorage or create a default root page
  const initialState = useMemo((): Page[] => {
    const saved = localStorage.getItem('notion-like-organizacao4-v1');
    if (saved) {
      try {
  ...
  useEffect(() => {
    localStorage.setItem('notion-like-organizacao4-v1', JSON.stringify(pages));
  }, [pages]);
        ...
        return (
          <div key={p.id} className="mb-1 pl-2">
            <div
              ...
              className={`flex items-center justify-between rounded px-2 py-1 cursor-pointer ${isActive ? 'bg-gray-200' : 'hover:bg-gray-100'
                }`}
              ...
              {activePage.blocks.map((block, idx) => {
                ...
                return (
                  <div
                    ...
                    className={`flex items-center gap-2 px-2 py-1 rounded mb-1 border ${isSelected ? 'bg-blue-50 border-blue-400' : isFocused ? 'bg-gray-50 border-gray-300' : 'border-transparent hover:bg-gray-50'
                      }`}
                    onMouseDown={(e) => {
                      // start selection
                      if (e.button !== 0) return;
                      // When dragging a selection, clicking inside a block should not start drag selection if the user intends to drag
                      setIsSelecting(true);
